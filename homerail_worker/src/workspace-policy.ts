...
import { normalizeWorkspaceAccess, workspacePathIsWritable, workspacePathWithin, protectedWorkspacePath, type DagWorkspaceAccess } from "homerail-protocol";

export interface WorkspaceSnapshot {
  files: Record<string, string>;
  /** Track creation/removal of excluded metadata roots without hashing Git internals. */
  metadata_roots?: string[];
}
...
function includesPath(prefixes: string[], candidate: string): boolean {
  return prefixes.some((prefix) => workspacePathWithin(candidate, prefix));
}
...
function digestSnapshot(snapshot: WorkspaceSnapshot): string {
  return digestFiles({ ...snapshot.files, ...Object.fromEntries((snapshot.metadata_roots ?? []).map(name => [name, "metadata:directory"])) });
}
...
export function snapshotWorkspace(root: string, policy: DagWorkspaceAccess): WorkspaceSnapshot {
  ...
  const access = normalizeWorkspaceAccess(policy);
  const writable = access.writable_paths;
  const readonly = access.readonly_paths ?? [];
  const gitMetadataRoots = new Set<string>([".git"]);
  for (const rootPath of readonly) {
    if (!includesPath(writable, rootPath)) gitMetadataRoots.add(rootPath === "." ? ".git" : `${rootPath}/.git`);
  }
  for (const rootPath of writable) {
    gitMetadataRoots.add(`${rootPath}/.git`);
  }
  ...
  const visit = (absolute: string, relative: string): void => {
    ...
    if (stat.isDirectory()) {
      if (gitMetadataRoots.has(relative)) { metadataRoots.push(relative); return; }
      if (relative === ".homerail-runtime" || path.posix.basename(relative) === "node_modules") return;
  ...
  return { files, metadata_roots: metadataRoots.sort() };
}
...
export function verifyWorkspacePolicy(
  ...
  const beforeMetadata = new Set(before.metadata_roots ?? []);
  const afterMetadata = new Set(after.metadata_roots ?? []);
  const metadataChanges = [...new Set([...beforeMetadata, ...afterMetadata])].filter(name => beforeMetadata.has(name) !== afterMetadata.has(name));
  const changed = [...new Set([...names].filter((name) => before.files[name] !== after.files[name]).concat(metadataChanges))].sort();
  const protectedChanges = changed.filter((name) => protectedWorkspacePath(name) || includesPath(readonly, name));
  const unauthorizedChanges = changed.filter((name) => !workspacePathIsWritable(name, access));
  return {
    ...
    before_hash: digestSnapshot(before),
    after_hash: digestSnapshot(after),
  };
}
